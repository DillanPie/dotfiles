...
void Playlist::shuffle() {
    ...
    int n = songCount;// Get the total number of songs total

    // Perform the shuffle algorithm 'n-1' times, where 'n' is the number of songs. This ensures that every song has a chance to be moved to any other position.
    for (int i = 0; i < n - 1; ++i) {
        // Generate a random index between 0 and 'i'. This will be used as the destination position for the current song.
        int j = i + std::rand() % (n - i);  // Random index

        // Find the 'jth' node in the playlist. We start at the head of the list and move forward 'j' steps, but we only need to go as far as 'i' because the destination position is guaranteed to be within this range.        
        Node* first = head;
